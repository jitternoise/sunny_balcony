...
func _unhandled_input(event: InputEvent) -> void:
	...
	if is_release:
		...
		if was_aiming:
			board.fire_catapult(_catapult_press_coord, _catapult_direction, _current_catapult_distance())
			return
...
func _update_catapult_aim() -> void:
	...
	board.set_catapult_aim(_catapult_press_coord, _catapult_direction, _current_catapult_distance())
...
func _handle_tap(screen_pos: Vector2) -> void:
	...
	# touches inventory, so no label refresh is needed -- and works both
	# before Start (extending the flow-preview arrows live) and while the
	# water is flowing (releasing backed-up water the moment the third tap
	# lands).
	if board.dig(coord):
		return
