...
## Revision-check fixes (2026-08-25, later same day)

A revision check over this session's code (see `claude/revision-check-2026-08-25.md`)
found seven defects; all are fixed in the files now at their canonical
`claude/game/scripts/...` paths. Still verified by inspection only — nothing has been
run in a live Godot window.

1. **A scroll drag that started on a catapult fired it.** `Level._process()` promoted a
   press into an aiming sequence on elapsed hold time alone, ignoring `_drag_active`, so
   scrolling a tall dig level from a catapult tile spent the shot on release. A press that
   passes `DRAG_THRESHOLD` now drops its catapult candidacy, guarded in both
   `_unhandled_input()` and `_process()`.
2. **A Bomb Catapult didn't actually block water like a Wall.** `_is_wall()` tested only
   `TickBehavior.WALL`, so natural fall *landed* water on the catapult's cell, where
   `_resolve_block_targets()`'s empty target list then held it permanently — instead of
   bouncing it onto the other diagonal the way a Wall does. `_is_wall()` now counts
   `CATAPULT` as solid, matching this document and `BlockData`'s own comment.
3. **Blocks could be placed on an activated Hydro Plant's cells.** `place_block()`'s
   "never on a water source" rule covered `water_sources` and `active_geysers` but not
   `hydro_source_cells`, and an activated plant's cells are `EMPTY` terrain, so the
   terrain guard didn't catch them either. `hydro_source_cells` is now in the same check.
4. **The pre-start flow preview forecast straight through an inactive plant.**
   `_predict_branch()`'s block-redirect loop was never updated alongside the two
   real-simulation loops; it now skips `_is_inactive_hydro()` targets the same way.
5. **Firing a preset catapult left a cell that swallowed blocks.** The coord stays in
   `preset_blocks` forever, so after `fire_catapult()` erased the block, `place_block()`
   allowed a new one there while `remove_block()` still refused to return it. Spent
   presets are now tracked in `_spent_preset_cells` and exempted from that guard.
6. **Nothing placed before Start existed.** Placements queued into `pending_placements`,
   and no PLACEMENT beat runs until the tick timer starts — so a planned block didn't
   render, didn't affect the flow preview (contradicting several doc comments), and a
   catapult couldn't be fired during planning. `HexBoard.started` (set by
   `Level._on_start_pressed()`) now makes place/remove commit immediately pre-Start.
   Post-Start buffering is untouched, and because beat 1 of measure 1 is PLACEMENT —
   which already ran before the first WATER beat — no level's simulation trace changes,
   so `level-solutions.md` and `level-min-times.md` stay valid.
7. **Two aim-preview papercuts.** At minimum charge the aim arrow's start and end
   coincided, so `_draw_flow_arrow()` normalized a zero vector and drew nothing for the
   first 350 ms of every shot; it now anchors at the catapult instead. And the fired
   distance is taken from the last previewed frame rather than recomputed at release,
   which could fire one tile past what the player was looking at.

Deliberately **not** changed, still open: a release always fires and always consumes the
block, even when the blast cluster contains no dirt at all, and there's no cancel gesture
once a hold has started. Both are game-feel decisions rather than defects.
