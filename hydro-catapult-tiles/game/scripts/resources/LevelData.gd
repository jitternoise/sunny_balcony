...
## times -- free, unlimited, any time before or after Start -- at which
...
## silently never render. Pointy-grid only for now -- untested and not yet
## supported on a "flat" level (see HexBoard.resolve_water_phase()'s hydro
## source-spawn loop, which always uses the pointy Hex.DOWN_LEFT start).
